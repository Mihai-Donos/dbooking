...
export default function Edit({
  ...
  const { data, setData, put, processing, errors, transform } = useForm({
    ...
    location_id: event?.location_id ?? (locations?.[0]?.id ?? ""),
    start_date: event?.start_date_local ?? "",
    end_date: event?.end_date_local ?? "",

    booking_visible_from: event?.booking_visible_from_local ?? "",
    booking_visible_to: event?.booking_visible_to_local ?? "",
    status: event?.status ?? (statusOptions?.[0]?.value ?? 0),
  ...
  const onSubmit = (e) => {
    ...
    put(route(`${routeBase}.update`, event.id), {
      preserveScroll: true,
  ...
  return (
    <AppShell
      ...
      subtitle={`#${event?.id ?? "—"} · Zeitraum, Sichtbarkeit, Status & Offerings anpassen.`}
      actions={
        <>
    ...
    >
      ...
      <section className="soft-surface p-6">
        <form id="event-edit-form" onSubmit={onSubmit} className="space-y-6">
          {/* Basics */}
